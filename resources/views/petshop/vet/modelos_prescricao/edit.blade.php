@extends('default.layout', ['title' => 'Editar Modelo de Prescrição'])

@section('content')
<x-form-page
    title="Editar Modelo de Prescrição"
    :back-url="route('vet.prescription-models.show', ['modeloPrescricao' => $modelo->id, 'page' => request()->query('page', 1)])"
>
            @php
                $availableCategories = $categorias;
                $categoryOptions = ['' => 'Selecione'] + $availableCategories;
                $statusOptions = $statusOptions ?? \App\Models\Petshop\ModeloPrescricao::statusOptions();
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-3">
                            {!! Form::select('status', 'Status', $statusOptions)
                                ->attrs(['class' => 'form-select'])
                                ->value(old('status', $modelo->status ?? \App\Models\Petshop\ModeloPrescricao::STATUS_ACTIVE)) !!}
                        </div>
