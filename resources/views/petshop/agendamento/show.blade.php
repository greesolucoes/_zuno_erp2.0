@extends('default.layout', ['title' => 'Agendamento'])
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-end">
                    <a href="{{ route('petshop.agenda.index', ['page' => request()->query('page', 1)]) }}" class="btn btn-danger btn-sm d-flex align-items-center gap-1 px-2">
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="mt-3" style="line-height: 0.7;">
                            ...
                            @isset($item->animal)
                              <p>
                                <b>Pet: <strong class="text-primary">{{ $item->animal->nome }}</strong></b>
                              </p>
                            @endif
                    ...
                    <div class="col-sm-4 offset-sm-2">
                        ...
                        <form method="POST" action="{{ route('petshop.agenda.update', [$item->id]) }}" class="mt-3 float-sm-end" style="line-height: 1.2;">
                            @method('put')
                            @csrf
                            <div class="col-12">
                ...
                <div class="row">
                    ...
                    <div class="d-print-none mt-4">
                        ...
                        <form action="{{ route('petshop.agenda.destroy', $item->id) }}" method="post" id="form-{{$item->id}}">
                            @method('delete')
                            @csrf
                        ...
                        <div class="text-end">
                            ...
                            <form method="post" action="{{ route('petshop.agenda.update-status', [$item->id]) }}" id="form-confirm-{{$item->id}}">
                                @method('PUT')
                                @csrf

                                @if($item->nfce_id == null)
                                @can('pdv_create')
                                <a href="{{ route('petshop.agenda.pdv', [$item->id]) }}" class="btn btn-dark">
