    ...
    setTimeout(() => {
        if(msg == ""){
            ...
            .done((res) => {
                ...
                new swal("Sucesso", "Cliente cadastrado!", "success")
                .then(() => {
            ...
            .fail((err) => {
                console.log(err)
                $('#modal_novo_cliente').modal('hide')
                new swal("Erro", "Erro ao cadastrar cliente: " + err.responseJSON, "error")
            })
        }else{
            new swal("Alerta", msg, "warning")
        }
    }, 300)
})
...
$(document).on("blur", "#inp-novo_cpf_cnpj", function () {
    ...
    if(cpf_cnpj.length == 11 && ($(this).data('cpf_autocomplet') === true)) {
        $.post(path_url + "api/clientes/find-customer", {
            cpf_cnpj: cpf_cnpj,
            empresa_id: empresa_id
        })
            .done((res) => {
                let nome = res.customer?.CADASTRAIS?.NOME ?? null;

                $('#inp-novo_razao_social').val(nome)
            })
            .fail((err) => {
    ...
    if(cpf_cnpj.length == 14) {
        ...
        .done((data) => {
            if (data!= null) {
                ...
                findClienteCidade(data.estabelecimento.cidade.ibge_id)
...
function findClienteCidade(codigo_ibge){
    $('#inp-cidade_id').html('')
...
$(document).on("blur", "#inp-novo_cep", function () {
    ...
    if(cep.length == 8){
        $.get('https://viacep.com.br/ws/' + cep + '/json/')
            .done((data) => {
                if (data!= null) {
                    $("#inp-novo_rua").val(data.logradouro)
                    $('#inp-novo_bairro').val(data.bairro)
                    $('#inp-novo_cidade_id').val(data.localidade)
                    findClienteCidade(data.ibge)
                }
            })
            .fail((err) => {
                console.log(err)
