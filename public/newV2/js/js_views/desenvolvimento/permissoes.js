...
function pesquisaPersonalizada() {
	let __acaoAtualizaDataTable = function () {
		...
		if(!is_empty($(modulo).val(), 1)) {
			if(!is_empty(gets_url, 1)) {
				gets_url += "&";
			}
			gets_url += "modulo=" + $(modulo).val();
		}
		if(!is_empty(gets_url, 1)) {
			url_table += "?" + gets_url;
		}
	...
	$("select#modulo").off("select2:select");
	$("select#modulo").on("select2:select", function () {
