function init(){
	...
	$("select.select_empresa").select2Ajax();
}
...
function pesquisaPersonalizada() {
	...
	var __preparaDadosParaBusca = (url_table, search) => {
		...
		let empresa = $("select#sel-empresa");
		let gets_url = "";

		if(!is_empty($(empresa).val(), 1)) {
			if(!is_empty(gets_url, 1)) {
				gets_url += "&";
			}
			gets_url += "empresas=" + $(empresa).val();
		}
	...
	$("select.select-search").off("select2:select");
	$("select.select-search").on("select2:select", function () {
		__acaoAtualizaDataTable();
	});

	$("select.select_empresa").on("select2:unselect", function () {
		$("select.select_empresa").val('')
		__acaoAtualizaDataTable();
	});
	...
	$("table.table-exibe").on('click', '.btn-open', function(){
		...
		let filiais = $(this).data('content')
		$("#modal-empresas-content").html(filiais)
		$("#modal-empresas-subtitle").html(`<h5><small class="text-secondary">flag:</small> <span class="badge badge-warning">${flag}</span></h5>`)
