function criaSelects(){
	$("select#controller").select2Simple();
	$("select#controller").select2Simple();
}
...
function pesquisaPersonalizada() {
	let __acaoAtualizaDataTable = function () {
		...
		let controller = $("select#controller");
		let url_table = "";
		let gets_url = "";
		let dataTable = null;
		...
		if(!is_empty($(controller).val(), 1)) {
			if(!is_empty(gets_url, 1)) {
				gets_url += "&";
			}
			gets_url += "controller=" + $(controller).val();
		}
		if(!is_empty(gets_url, 1)) {
			url_table += "?" + gets_url;
		}
	...
	$("select#controller").off("select2:select");
	$("select#controller").on("select2:select", function () {
