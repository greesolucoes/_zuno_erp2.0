        ...
        $($('table#conteudoTable tbody').find('tr')).each(function () {
            if(iMoeda === 0 || isFirstTime) {
                if(!isFirstTime){
                    ...
                    $(this).find('select.select_utilizacao').append('<option value=""></option>');
                    ...
                    $(this).find('select.select_utilizacao option[value=""]').prop('selected', 'selected');
...
function trocaParametroCamposMaskNumerosV2(__this) {
    // console.log($(__this).next().length);
    // console.log($(__this).prev().length);
    // console.log($(__this).parent().length);
...
function ajaxSelectUM(){
    ...
    $(".select_produtoId").change(function (){
        ...
        if(travaTriggerProd != 1){
			...
			//Utilização automatica por produto
			var paramUtilizacaoAutomatica = $('.data_views').data('utilizacao_automatica');
			if(paramUtilizacaoAutomatica){
				ajaxUtilizacaoProduto(produto, $(this).parents('tr').find('select.select_utilizacao'))
			}
        }
...
function ajaxUtilizacaoProduto(produto, selectUtilizacao){
	var urlJsonUtilizacao = $('.data_views').data('url_json_utilizacao');
	if(produto !== null && produto !== '') {
		ajaxRequest(true, urlJsonUtilizacao, null, 'text', {'produto': produto}, function(ret){
			if(ret){
				ret = $.parseJSON(ret);
				$(selectUtilizacao).find('option[value="' + ret['id'] + '"]').prop('selected', true);
				$(selectUtilizacao).trigger('change');
				$(selectUtilizacao).find('option:not(:selected)').remove();
			}else{
				$(selectUtilizacao).append('<option value=""></option>');
				$(selectUtilizacao).find('option[value=""]').prop('selected', true);
			}
		});
	}
}
...
function addClickButtonModal(){
	...
	// funcao executada ao clica para inserir os itens do grupo
    ...
    $('.modal.modalItensPorGrupo button.btnModal').click(function () {
		...
		// itera em todas as linhas de itens do grupo para remover as linhas com qtde vazias
        $(this).parents('.modal.modalItensPorGrupo').find('table#table-modal tbody tr').each(function () {
			...
			// itera em todas as linhas de itens da tela inicial
            $("table#conteudoTable tbody tr").each(function () {
				// se a linha do item ja existir na tela principal, remove para inserir uma nova
                if(dataIdProduto == $(this).find('select.select_produtoId').val()){
                    $(this).remove();
                }
				// se a linha atual tiver qtde vazia, checa se a linha na table principal está vazia tbm e remove da tela principal
                if(delRowEmpty == 1){
				...
				$("table#conteudoTable tbody").append('' +
                    '<tr>' +
                        ...
                        '<td>' +
                            ...
                            'data-maxdecimal="" ' +
							'data-thousand_delimiter="' + configLocation.currencyThousandsSeparator + '" ' +
							'data-decimal_delimiter="' + configLocation.currencyDecimalPoint + '" ' +
						...
						'<td>' +
							'<select class="form-control select select_deposito" name="deposito[]" style="width: 100%;">' +
								'<option value="" selected="selected">Depósito Padrão</option>' +
							'</select>' +
						'</td>' +
						htmlRegraDistr +
						htmlDespesasAdicionais +
					//adicionar retorno do brn incluir...
						'<td>' +
                        ...
                        '<td> ' +
                            ...
                            `<button class="${isOldLayout ? 'btn btn-primary' : 'button-form primary-button'} controlar_lote ${(is_empty(retAjax.isControlarLote, 1) ? ' ocultar' : '')}" 
									 type="button" title="${$(".data_views").data("lang_btn_lote")}">
				...
				if(!is_empty(flagUtilizarDespesasAdicionaisLinhaItem,1)) {
					...
					$.each(despesasAdicionais, function (idDespesasAd, valueDespesasAdicionais) {
						$($("table#conteudoTable tbody tr select.conteudo-itens_despesa1").last()).append('' +
							'<option value="' + valueDespesasAdicionais.id + '">' + valueDespesasAdicionais.text + '</option>'
						);
					});
					// Despesas 2
					$.each(despesasAdicionais, function (idDespesasAd, valueDespesasAdicionais) {
	...
	/*
	* Carrega os dados em tela para dentro do model de grupos...
	* */
function setItensModalGrupo(){
    ...
    $("button.grupoItens").click(function (){
        ...
        var fornecedor       = $('select.select_fornecedorId').val();
        ...
        if(fornecedor !== null && fornecedor !== '' && fornecedor !== undefined){
            if(idGrupo !== null && idGrupo !== '') {
                ...
                $("table#conteudoTable tbody tr").each(function () {
                    linhasProdutos.push({
                        idProduto: 				$(this).find("select.select_produtoId").val(),
                        ...
                        utilizacao: 			$(this).find(".select_utilizacao option:selected").val(),
...
/**
 * funcao para criar nova linha de produto/item
 */
$('table#conteudoTable button.addItens').click(function () {
	// pega a primeira linha da table como modelo
    var modelo   = $('table#conteudoTable tbody tr').first().html();

	// insere a nova linha e remove buttons e reseta os selects
	...
	// pego a ultima tr que acabou de ser criada para remontar os campos
    ...
    $(limpaCampos).find('select.select_utilizacao').append('<option value=""></option>');
    ...
    $(limpaCampos).find('select.select_utilizacao option[value=""]').prop('selected', 'selected');
